...
void commit_list_dump(const commit_list_t *const commit_list,
                      FILE *output_file) {
  ...
  for (m_list_iterator_go_to_head(commit_iterator);
    ...
    for (m_list_iterator_go_to_head(change_iterator);
      ...
      printf("    New file: %s, old file: %s, type: %s, added: %d, "
             "deleted: %d\n",
             change->new_file, change->old_file, type, change->added_lines,
...
void commit_list_fetch_local(commit_list_t *commit_list, const char *path) {
  ...
  do {
    ...
    if (old_last_commit_id &&
        strcmp(old_last_commit_id, git_oid_tostr_s(git_commit_id(parent))) ==
            0) {
    ...
    error = git_diff_tree_to_tree(&diff, repository, parent_tree, commit_tree,
                                  NULL);
    if (error < 0) {
      printf("6 %d\n", error);
      break;
    ...
    error = git_diff_foreach(diff, each_file_cb, each_binary_cb, each_hunk_cb,
                             each_line_cb, commit);
    if (error < 0) {
      printf("7 %d\n", error);
      break;
