...
void commit_list_destroy(commit_list_t **commit_list)
{
    ...
    for (m_list_iterator_go_to_head(commit_iterator);
...
void commit_list_dump(const commit_list_t *const commit_list, FILE *output_file)
{
    ...
    fwrite(&commit_list->commit_list->size,
           sizeof(commit_list->commit_list->size), 1, output_file);
    commit_iterator = m_list_iterator_create(commit_list->commit_list);
    for (m_list_iterator_go_to_head(commit_iterator);
         (tmp = m_list_iterator_current(commit_iterator)) != NULL;
         m_list_iterator_next(commit_iterator))
    {
        ...
        fwrite(&commit->change_list->size, sizeof(commit->change_list->size), 1,
               output_file);
        change_iterator = m_list_iterator_create(commit->change_list);
        for (m_list_iterator_go_to_head(change_iterator);
             (tmp = m_list_iterator_current(change_iterator)) != NULL;
             m_list_iterator_next(change_iterator))
        {
            ...
            fwrite(&change->added_lines, sizeof(change->added_lines), 1,
                   output_file);
            fwrite(&change->deleted_lines, sizeof(change->deleted_lines), 1,
                   output_file);
        }
        m_list_iterator_destroy(&change_iterator);
    }
...
void commit_list_load(commit_list_t *commit_list, FILE *input_file)
{
    ...
    commit_list->last_commit_id =
        (char *)m_mem_calloc(tmp_str_len + 1, sizeof(char));
    ...
    for (uint32_t i = 0; i < commit_list_size; i++)
    {
        ...
        for (uint32_t j = 0; j < change_list_size; j++)
        {
            ...
            fread(&change->added_lines, sizeof(change->added_lines), 1,
                  input_file);
            fread(&change->deleted_lines, sizeof(change->deleted_lines), 1,
                  input_file);
         ...
         m_list_iterator_next(commit_iterator))
    {
             ...
             m_list_iterator_next(change_iterator))
        {
            ...
            printf("    New file: %s, old file: %s, type: %s, added: %d, "
                   "deleted: %d\n",
                   change->new_file, change->old_file, type,
...
void commit_list_fetch_local(commit_list_t *commit_list, const char *path)
{
    ...
    commit_list->last_commit_id =
        strdup(git_oid_tostr_s(git_commit_id(git_commit)));
    if (old_last_commit_id &&
        strcmp(old_last_commit_id, commit_list->last_commit_id) == 0)
    {
    ...
    do
    {
        ...
        if (old_last_commit_id &&
            strcmp(old_last_commit_id,
                   git_oid_tostr_s(git_commit_id(parent))) == 0)
        {
        ...
        error = git_diff_foreach(diff, each_file_cb, each_binary_cb,
                                 each_hunk_cb, each_line_cb, commit);
        if (error < 0)
        {
            printf("7 %d\n", error);
            break;
