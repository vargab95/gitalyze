...
m_args_t *get_args(int argc, char **argv)
{
    ...
    m_args_add_entry(args, (m_args_entry_t){.id = ARG_REPOSITORY_URL,
                                            ...
                                            .long_switch = "--url",
                                            .environment_variable =
                                                "GITALYZE_REPOSITORY_URL",
                                            .flags = {.required = 0},
                                            .expected_type = ARG_TYPE_STRING,
                                            .preference = ARG_PREFER_SHORT});

    m_args_add_entry(args,
                     (m_args_entry_t){.id = ARG_COMMIT_LIST_PATH,
                                      .short_switch = "-c",
                                      .long_switch = "--commit-list-cache",
                                      .environment_variable =
                                          "GITALYZE_COMMIT_LIST_CACHE_PATH",
                                      .flags = {.required = 0},
                                      .expected_type = ARG_TYPE_STRING,
                                      .preference = ARG_PREFER_SHORT});

    m_args_add_entry(args,
                     (m_args_entry_t){.id = ARG_OBJECT_TREE_PATH,
                                      .short_switch = "-o",
                                      .long_switch = "--object-tree-cache",
                                      .environment_variable =
                                          "GITALYZE_OBJECT_TREE_CACHE_PATH",
                                      .flags = {.required = 0},
                                      .expected_type = ARG_TYPE_STRING,
                                      .preference = ARG_PREFER_SHORT});

    m_args_add_entry(
        args, (m_args_entry_t){.id = ARG_VERBOSE,
                               .short_switch = "-v",
                               .long_switch = "--verbose",
...
int process_commit_list(commit_list_t *commit_list, m_args_t *args)
{
    ...
    if ((arg_entry = m_args_get(args, ARG_REPOSITORY_PATH)) &&
        arg_entry->flags.present)
    {
